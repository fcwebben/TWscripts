 ...
 * - Routes every script-owned GET/POST through one shared network limiter (max 5 request starts/second)
 * - Caches non-volatile world/account metadata (group list and group membership coordinates) for one hour
 * - Does not cache live resources, merchants or incoming transports, because those values must stay current
 *
 ...
 *
 * Script Library review compliance:
 * - Shared request limit: all fetch() and TribalWars.post() calls created by this script pass through the same limiter.
...
(function () {
  ...
  // 220 ms is intentionally slightly more conservative than the 200 ms theoretical 5 req/s boundary.
  ...
  function scheduleNetworkRequest(label, task) {
    const gate = networkStartQueue.then(async function () {
      ...
      if (typeof console !== "undefined" && console.debug) {
        console.debug(
          SCRIPT_NAME + " network start",
          { label: label, maxRequestsPerSecond: NETWORK_MAX_REQUESTS_PER_SECOND }
        );
      }
    });

    // Advance the start gate independently of request completion so slow responses do not block later starts.
  ...
  async function loadBaseDataAndRender() {
    try {
      ...
      setStatus(
        "Ready. Loaded " + state.production.size + " own villages and " + state.groups.length +
        " groups. Shared network limit: max 5 req/s. Static group data cache: 60 min.",
        "success"
      );
    } catch (err) {
