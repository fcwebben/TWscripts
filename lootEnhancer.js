...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const getCommandTargetKeys = function (coord, targetId) {
    ...
    if (coord) keys.push('coord:' + coord);
    if (targetId) keys.push('id:' + targetId);

    // Backwards compatibility with older code that stored commands directly by coordinate.
    if (coord) keys.push(coord);

    return [...new Set(keys)];
  };

  const addCommandArrival = function (commands, coord, targetId, arrivalTimestamp) {
    if (!arrivalTimestamp) return;

    getCommandTargetKeys(coord, targetId).forEach((key) => {
      if (!commands.hasOwnProperty(key)) commands[key] = [];
      commands[key].push(arrivalTimestamp);
    });
  };

  const getCommandArrivals = function (commands, coord, targetId) {
    let seen = new Set();
    let arrivals = [];

    getCommandTargetKeys(coord, targetId).forEach((key) => {
      (commands[key] || []).forEach((timestamp) => {
  ...
  const getCommandTargetId = function ($row) {
    let href = $row
      .find('a[href*="screen=info_village"], a[href*="info_village"]')
      .first()
      .attr('href') || '';
  ...
  const getCommandArrivalText = function ($row) {
    let cells = $row.find('td').toArray();

    for (let i = 0; i < cells.length; i++) {
      let text = $(cells[i]).text().trim();

      if (/today|tomorrow|\d{1,2}[.]\d{1,2}[.]|\d{1,2}\/\d{1,2}\//i.test(text)) {
        return text;
      }
    }
  ...
  const parseCommandArrivalTimestamp = function (arrivalText) {
    try {
      let timestamp = Math.round(lib.timestampFromString(arrivalText) / 1000);
  ...
  const createPlanning = function (
    ...
    for (let prop in data.villages) {
      ...
      orderedFarms.forEach((el) => {
        ...
        let targetId = farmIndex.id;
        let commandArrivals = getCommandArrivals(data.commands, el.coord, targetId);
        let timeDiff = true;

        if (!farmIndex.hasOwnProperty('color') && commandArrivals.length > 0) {
          timeDiff = false;
        }
        ...
        if (unitsLeft && timeDiff && distance < optionDistance) {
          ...
          addCommandArrival(data.commands, el.coord, targetId, arrival);
        }
      });
    }
